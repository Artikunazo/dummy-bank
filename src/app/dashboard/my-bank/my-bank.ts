				...
				@for (item of userProducts(); track item.id) {
					<app-card class="w-100">
						<h3 class="text-lg flex justify-between">
							{{ item.name }}
							<span class="text-right">{{ item.balance ?? 0 | currency }}</span>
						</h3>
						<hr class="text-gray-300" />
						<div class="card-footer flex justify-between gap-4">
							@if (item.card) {
								<button class="btn btn-primary mt-2" title="View card details">
									<svg class="w-6 h-6 card-icon">
										<use href="/assets/credit-card.svg" />
									</svg>
								</button>
							}
							<button class="btn btn-primary mt-2" title="Deposit funds">
								<svg class="w-6 h-6 card-icon">
									<use href="/assets/deposit-founds.svg" />
								</svg>
							</button>
							<button class="btn btn-primary mt-2" title="Withdraw funds">
								<svg class="w-6 h-6 card-icon">
									<use href="/assets/withdraw-founds.svg" />
								</svg>
							</button>
							@if (item.card && !item.card.isLocked) {
								<button class="btn btn-primary mt-2" title="Unlock card">
									<svg class="w-6 h-6 card-icon">
										<use href="/assets/unlock.svg" />
									</svg>
								</button>
							} @else {
								<button class="btn btn-primary mt-2" title="Lock card">
									<svg class="w-6 h-6 card-icon">
										<use href="/assets/lock.svg" />
									</svg>
								</button>
							}
						</div>
